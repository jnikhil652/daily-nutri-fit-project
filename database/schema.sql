-- Daily Nutri Fit Database Schema
...
DROP TRIGGER IF EXISTS delivery_addresses_updated_at ON delivery_addresses;
...
DROP TRIGGER IF EXISTS ensure_single_primary_address_trigger ON delivery_addresses;
...
CREATE TABLE IF NOT EXISTS fruit_category_mappings (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fruit_nutritional_info (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS fruit_health_benefits (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  fruit_id UUID REFERENCES fruits(id) ON DELETE CASCADE NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_fruits_availability ON fruits(is_available) WHERE is_available = true;
CREATE INDEX IF NOT EXISTS idx_fruits_name_search ON fruits USING gin(to_tsvector('english', name || ' ' || description));
CREATE INDEX IF NOT EXISTS idx_fruits_price ON fruits(price);
CREATE INDEX IF NOT EXISTS idx_fruit_categories_name ON fruit_categories(name);
CREATE INDEX IF NOT EXISTS idx_user_favorites_user ON user_favorites(user_id);
CREATE INDEX IF NOT EXISTS idx_nutritional_calories ON fruit_nutritional_info(calories_per_100g);
CREATE INDEX IF NOT EXISTS idx_nutritional_vitamin_c ON fruit_nutritional_info(vitamin_c_mg);
...
DROP TRIGGER IF EXISTS fruits_updated_at ON fruits;
...
AND NOT EXISTS (
  SELECT 1 FROM fruit_nutritional_info WHERE fruit_id = f.id
);
...
WHERE 
  ...
  AND NOT EXISTS (
    SELECT 1 FROM fruit_category_mappings WHERE fruit_id = f.id AND category_id = c.id
  );
...
WHERE 
  ...
  AND NOT EXISTS (
    SELECT 1 FROM fruit_health_benefits WHERE fruit_id = f.id AND benefit_id = b.id
  );
