...
This is the database schema implementation for the spec detailed in @.agent-os/specs/2025-08-14-basic-ordering/spec.md
...
**shopping_carts** - User shopping cart data with persistence
...
**Cart Persistence:** Shopping cart data is stored in the database to enable cross-device synchronization and prevent cart abandonment.

**Order Immutability:** Once created, order items store snapshot data of fruits to maintain historical accuracy even if fruit information changes.

**Atomic Order Creation:** The `create_order_from_cart` function ensures cart-to-order conversion is atomic, preventing data inconsistency.

**Order Number Generation:** Human-readable order numbers with date prefix for easy customer reference and support.

**Flexible Delivery Scheduling:** Configurable time slots allow for future expansion of delivery options and capacity management.